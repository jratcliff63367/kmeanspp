...
public:
    ...
    /**
    * Performs kmeans operation on the input set of data points and returns the results.
    *
    * @param sourcePoints : Pointer to an array of 3d data points (X,Y,Z) as double precision floats
    * @param pointCount : The number of input data points
    * @param maxPoints : The maximum number of output data points we are allowed to produce
    * @param resultPointCount : A reference to return how many points were created.
    * 
    * @return : Returns a pointer to the reduced mean points.
    */
    virtual const double *compute(const double *sourcePoints,
                                  uint32_t pointCount,
                                  uint32_t maxPoints,
...
class KmeansPPImpl : public KmeansPP
{
public:
    KmeansPPImpl(void)
    {
    ...
    */
                                   ...
                                   uint32_t &resultPointCount) final
    {
        ...
        {
            const double *p = &sourcePoints[i*3];
            Point3d pp;
            pp[0] = p[0];
            pp[1] = p[1];
            pp[2] = p[2];
        ...
        mResultsD = std::get<0>(reduced);

        resultPointCount = uint32_t(mResultsD.size());
        if ( !mResultsD.empty() )
        {
            ret = &mResultsD[0][0];
        }
        ...
        uint32_t &resultPointCount) final
    {
        ...
        resultPointCount = uint32_t(mResultsF.size());
        if (!mResultsF.empty())
        {
            ret = &mResultsF[0][0];
        }
    ...
    Point3dVector    mResultsD;
    Point3fVector    mResultsF;
};

KmeansPP *KmeansPP::create(void)
{
    auto ret = new KmeansPPImpl;
